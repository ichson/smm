const { Error, TypeError } = require('../error.js');
...
module.exports = async(webhook, messageId, options, info) => {
    ...
    const response = await fetch(`https://discord.com/api/v9/webhooks/${(await webhook).id}/${(await webhook).token}/messages/${messageId}`, {
        method: 'PATCH',
        headers: {
